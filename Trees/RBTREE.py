...
class RedBlackTree:
    ...
    def prin(self):
        ...
        def inner_find_right(root):
            global lo, count
            if root is None:
                return
            if 'NIL' in str(root):
                count += 1
            if root.left is not None and lo >= 1:
                print(' ' * 20, root.left, "L")
                if 'NIL' in str(root.left):
                    count += 1
            print(' ' * 20, root.right, "R")
            lo += 1

            return inner_find_right(root.right)

        lo = 0

        def inner_find_left(root):
            global lo, count
            if root is None:
                return
            if 'NIL' in str(root):
            ...
            if root.right is not None and lo >= 1:
                print(root.right, 'R')
                if 'NIL' in str(root.right):
                    count += 1

            print(root.left, 'L')
            lo += 1
            return inner_find_left(root.left)
        inner_find_left(self.root)
        inner_find_right(self.root)
        print(count)
